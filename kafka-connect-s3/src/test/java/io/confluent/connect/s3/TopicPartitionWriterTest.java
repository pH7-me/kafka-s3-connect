...
public class TopicPartitionWriterTest extends TestWithMockedS3 {
  ...
  public void testWriteRecordDefaultWithPaddingSeq() throws Exception {
    ...
    expectedFiles.add(
        FileUtils.fileKeyToCommit(topicsDir, dirPrefix, TOPIC_PARTITION, 0, extension, "%02d"));
    expectedFiles.add(
        FileUtils.fileKeyToCommit(topicsDir, dirPrefix, TOPIC_PARTITION, 3, extension, "%02d"));
    expectedFiles.add(
        FileUtils.fileKeyToCommit(topicsDir, dirPrefix, TOPIC_PARTITION, 6, extension, "%02d"));
  ...
  public void testWriteRecordFieldPartitioner() throws Exception {
    ...
    String dirPrefix1 =
        partitioner.generatePartitionedPath(TOPIC, partitionField + "=" + String.valueOf(16));
    String dirPrefix2 =
        partitioner.generatePartitionedPath(TOPIC, partitionField + "=" + String.valueOf(17));
    String dirPrefix3 =
        partitioner.generatePartitionedPath(TOPIC, partitionField + "=" + String.valueOf(18));

    List<Struct> expectedRecords = new ArrayList<>();
    int ibase = 16;
    float fbase = 12.2f;
    // The expected sequence of records is constructed taking into account that sorting of files
    // occurs in verify
    for (int i = 0; i < 3; ++i) {
      for (int j = 0; j < 6; ++j) {
  ...
  public void testWriteRecordTimeBasedPartitionWallclockRealtime() throws Exception {
    ...
    parsedConfig.put(
        ...
        "'year'=YYYY_'month'=MM_'day'=dd_'hour'=HH_'min'=mm_'sec'=ss");
    partitioner.configure(parsedConfig);
    ...
    for (int i : new int[] {0, 3, 6}) {
      expectedFiles.add(
          FileUtils.fileKeyToCommit(
              topicsDir, dirPrefixFirst, TOPIC_PARTITION, i, extension, ZERO_PAD_FMT));
    }
    ...
    for (int i : new int[] {9, 12, 15}) {
      expectedFiles.add(
          FileUtils.fileKeyToCommit(
              topicsDir, dirPrefixLater, TOPIC_PARTITION, i, extension, ZERO_PAD_FMT));
    }
    verify(expectedFiles, 3, schema, records);
  }
  ...
  public void testWriteRecordTimeBasedPartitionRecordTimestampHours() throws Exception {
    ...
    Collection<SinkRecord> sinkRecords =
        createSinkRecordsWithTimestamp(
            records.subList(0, 9), key, schema, 0, timestampFirst, advanceMs);
    long timestampLater = first.plusHours(2).getMillis();
    sinkRecords.addAll(
        createSinkRecordsWithTimestamp(
            records.subList(9, 18), key, schema, 9, timestampLater, advanceMs));
  ...
  public void testWallclockUsesBatchTimePartitionBoundary() throws Exception {
    ...
    List<String> expectedFiles = new ArrayList<>();
    String dirPrefix =
        partitioner.generatePartitionedPath(TOPIC, getTimebasedEncodedPartition(freezeTime));
    expectedFiles.add(
        FileUtils.fileKeyToCommit(
  ...
  private String getTimebasedEncodedPartition(long timestamp) {
    long partitionDurationMs =
        (Long) parsedConfig.get(PartitionerConfig.PARTITION_DURATION_MS_CONFIG);
  ...
  public void testWriteRecordDefaultWithEmptyTopicsDir() throws Exception {
    ...
    expectedFiles.add(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 3, extension, ZERO_PAD_FMT));
    expectedFiles.add(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 6, extension, ZERO_PAD_FMT));
    verify(expectedFiles, 3, schema, records);
  }
  ...
  public void testAddingS3ObjectTags() throws Exception {
    ...
    expectedTaggedFiles.put(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 0, extension, ZERO_PAD_FMT),
        Arrays.asList(
            new Tag("startOffset", "0"), new Tag("endOffset", "2"), new Tag("recordCount", "3")));
    expectedTaggedFiles.put(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 3, extension, ZERO_PAD_FMT),
        Arrays.asList(
            new Tag("startOffset", "3"), new Tag("endOffset", "5"), new Tag("recordCount", "3")));
    expectedTaggedFiles.put(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 6, extension, ZERO_PAD_FMT),
        Arrays.asList(
            new Tag("startOffset", "6"), new Tag("endOffset", "8"), new Tag("recordCount", "3")));
  ...
  public void testExceptionOnNullKeysReported() throws Exception {
    ...
    String exceptionMessage =
        String.format("Key cannot be null for SinkRecord: %s", faultyRecord.toString());
  ...
  public void testExceptionOnEmptyHeadersReported() throws Exception {
    ...
    String exceptionMessage =
        String.format("Headers cannot be null for SinkRecord: %s", faultyRecord.toString());
    testExceptionReportedToDLQ(faultyRecord, DataException.class, exceptionMessage, false, false);
    tearDown(); // clear mock S3 port for follow up test
    // test with faulty being first in batch
    testExceptionReportedToDLQ(faultyRecord, DataException.class, exceptionMessage, true, false);
  ...
  public void testSchemaProjectionExceptionReported() throws Exception {
    ...
    testExceptionReportedToDLQ(
        faultyRecord, SchemaProjectorException.class, exceptionMessage, false, true);
  }
  ...
  public void testPartitioningExceptionReported() throws Exception {
    ...
    topicPartitionWriter.buffer(
        new SinkRecord(TOPIC, PARTITION, Schema.STRING_SCHEMA, "key", schema, struct, 0));
    // non-struct record should throw exception and get reported
    topicPartitionWriter.buffer(
        new SinkRecord(TOPIC, PARTITION, Schema.STRING_SCHEMA, "key", schema, "not a struct", 1));
  ...
  public void testRecordKeysAndHeadersWritten() throws Exception {
    ...
    expectedHeaderFiles.add(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 0, ".headers.avro", ZERO_PAD_FMT));
    expectedHeaderFiles.add(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 3, ".headers.avro", ZERO_PAD_FMT));
    expectedHeaderFiles.add(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 6, ".headers.avro", ZERO_PAD_FMT));
    verifyRecordElement(expectedHeaderFiles, 3, sinkRecords, RecordElement.HEADERS);

    List<String> expectedKeyFiles = new ArrayList<>();
    expectedKeyFiles.add(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 0, ".keys.avro", ZERO_PAD_FMT));
    expectedKeyFiles.add(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 3, ".keys.avro", ZERO_PAD_FMT));
    expectedKeyFiles.add(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 6, ".keys.avro", ZERO_PAD_FMT));
  ...
  public void testRecordHeadersWrittenJson() throws Exception {
    ...
    TopicPartitionWriter topicPartitionWriter =
        new TopicPartitionWriter(
            TOPIC_PARTITION,
            storage,
            getKeyHeaderValueProviderJsonHeaders(),
            partitioner,
            connectorConfig,
            context,
            null);
    ...
    expectedHeaderFiles.add(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 0, HEADER_JSON_EXT, ZERO_PAD_FMT));
    expectedHeaderFiles.add(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 3, HEADER_JSON_EXT, ZERO_PAD_FMT));
    expectedHeaderFiles.add(
        FileUtils.fileKeyToCommit(
            topicsDir, dirPrefix, TOPIC_PARTITION, 6, HEADER_JSON_EXT, ZERO_PAD_FMT));
    verifyRecordElement(expectedHeaderFiles, 3, sinkRecords, RecordElement.HEADERS);
  }
  ...
  private RecordWriterProvider<S3SinkConnectorConfig> getKeyHeaderValueProvider() {
    ...
    return new KeyValueHeaderRecordWriterProvider(
        new AvroFormat(storage).getRecordWriterProvider(), keyWriterProvider, headerWriterProvider);
  }
  ...
  private RecordWriterProvider<S3SinkConnectorConfig> getKeyHeaderValueProviderJsonHeaders() {
    ...
    // initialize the KVHWriterProvider with header and key writers turned on.
    return new KeyValueHeaderRecordWriterProvider(
        new AvroFormat(storage).getRecordWriterProvider(), null, headerWriterProvider);
  }
  ...
  // Create a list of records by repeating the same record batch. Total number of records:
  // 'batchesNum' x 'batchSize'
  ...
  // Given a list of records, create a list of sink records with contiguous offsets.
  private List<SinkRecord> createSinkRecordsWithHeaders(
      List<Struct> records, String key, Schema schema) {
    return createSinkRecordsWithHeaders(records, key, schema, 0);
  }

  // Given a list of records, create a list of sink records with contiguous offsets.
  private List<SinkRecord> createSinkRecords(
      List<Struct> records, String key, Schema schema, int startOffset) {
  ...
  // Given a list of records, create a list of sink records with contiguous offsets.
  private List<SinkRecord> createSinkRecordsWithTimestamp(
      List<Struct> records,
      String key,
      Schema schema,
      int startOffset,
  ...
  private void verify(
      List<String> expectedFileKeys, int expectedSize, Schema schema, List<Struct> records)
      throws IOException {
  ...
  // based on verify()
  private void verifyRecordElement(
      List<String> expectedFileKeys,
      int expectedSize,
      List<SinkRecord> records,
      RecordElement fileType)
      throws IOException {
    ...
    for (String fileKey : actualFiles) {
      ...
      for (Object avroRecord : actualRecords) {
        ...
        if (fileKey.endsWith(HEADER_AVRO_EXT)) {
          ...
          expectedRecord =
              ((NonRecordContainer) format.getAvroData().fromConnectData(headerSchema, value))
                  .getValue();
          ...
          byte[] jsonBytes =
              configuredJsonConverter().fromConnectData(currentRecord.topic(), headerSchema, value);
          ...
          expectedRecord =
              ((NonRecordContainer)
                      format
                          .getAvroData()
                          .fromConnectData(currentRecord.keySchema(), currentRecord.key()))
                  .getValue();
          expectedRecord =
              new Utf8(
                  (String) expectedRecord); // fix assert conflicts due to java string and avro utf8
        } else {
          expectedRecord =
              format
                  .getAvroData()
                  .fromConnectData(currentRecord.valueSchema(), currentRecord.value());
        }
        assertEquals(expectedRecord, avroRecord);
      }
    }
  ...
  private List<String> getS3FileListValues(List<S3ObjectSummary> summaries) {
    Set<String> excludeExtensions =
        new HashSet<>(Arrays.asList(HEADER_AVRO_EXT, HEADER_JSON_EXT, KEYS_AVRO_EXT));
  ...
  private void verifyTags(Map<String, List<Tag>> expectedTaggedFiles) throws IOException {
    List<S3ObjectSummary> summaries = listObjects(S3_TEST_BUCKET_NAME, null, s3);
    List<String> actualFiles = new ArrayList<>();
    for (S3ObjectSummary summary : summaries) {
      String fileKey = summary.getKey();
