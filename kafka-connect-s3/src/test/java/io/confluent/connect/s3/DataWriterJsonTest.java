...
public class DataWriterJsonTest extends DataWriterTestBase<JsonFormat> {
  ...
  public void testNoSchema() throws Exception {
    ...
    List<SinkRecord> sinkRecords =
        createJsonRecordsWithoutSchema(7 * context.assignment().size(), 0, context.assignment());
    task.put(sinkRecords);
    task.close(context.assignment());
    task.stop();
  ...
  protected void verifyContents(
    ...
    for (Object jsonRecord : records) {
      ...
      if (expectedValue instanceof Struct) {
        byte[] expectedBytes =
            converter.fromConnectData(TOPIC, expectedRecord.valueSchema(), expectedRecord.value());
